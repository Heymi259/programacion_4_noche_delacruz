    ...
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        ...
        child: Row(
          ...
          children: [
            Text(
              'Volumen total: \$${state.totalWithTax.toStringAsFixed(2)}',
              style: const TextStyle(
                fontSize: 16,
                fontWeight: FontWeight.bold,
                color: AppColors.textPrimary,
...
Future<void> _confirmOrder(BuildContext context, WidgetRef ref, CartState state) async {
  ...
  if (!authState.isAuthenticated) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Debes iniciar sesión para confirmar una sesión')),
    );
    context.go('/login');
    return;
  }
  ...
  final shouldConfirm = await showDialog<bool>(
    ...
    builder: (dialogContext) => AlertDialog(
      title: const Text('Confirmar sesión'),
      content: const Text('¿Deseas confirmar esta sesión?'),
      actions: [
        TextButton(
  ...
  try {
    ...
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Sesión #${confirmedOrder.id} confirmada')),
    );
    context.go('/orders');
  } catch (e) {
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Error confirmando sesión: $e')),
    );
  }
}