...
@Composable
fun PantallaFavoritosContent_mp(
    favoritos: List<Contacto_mp>,
    modifier:  Modifier = Modifier
) {
    if (favoritos.isEmpty()) {
        ...
        LazyColumn(
            ...
            items(favoritos, key = { it.id }) { contacto ->
                TarjetaContacto_mp(contacto = contacto)
            }
        }
    }
}
...
@Composable
fun PantallaPerfilContent_mp(modifier: Modifier = Modifier) {
    Box(modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
...
fun Paso05_Preview_mp() {
    MaterialTheme { Paso05NavBarScreen_mp() }
}
